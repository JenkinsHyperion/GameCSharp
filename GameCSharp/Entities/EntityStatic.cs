...
{
    public class EntityStatic : Entity
    {


        private Scene.LayeredEntity sceneSlot;
        ...
        public EntityStatic(int x, int y) : base(x, y)
        {
            //isSelected = false;
            init();
        }
    ...
    //COMPOSITE CONTRUCTION
        public EntityStatic(String name, int x, int y) : base(x, y)
        {
            this.name = name;
            init();
        }

        public EntityStatic(String name, Point position) : base(x, y)
        {
            this.name = name;
            init();
        }

        public EntityStatic(Point entityPosition) : base(x, y)
        {
            init();
        }
        ...
        /** @return Collider Composite if available, otherwise ColliderNull
	     */
        ...
        { // gets the Object's sprite, still image or animation MOVE TO SPRITEPROPERTY
        ...
        public void disable()
        {
            ...
            for (UpdateableComposite remove : removals)
            {
                ...
                if (!remove.removeThisUpdateableComposite())
                { //Remove math calculations from updater thread
        ...
        {
            return this.sceneSlot.getLayer();
        }
        ...
        public void addEntityToScene(Scene scene, byte layer)
        {
            if (this.sceneSlot == null)
            {
                sceneSlot = scene.registerEntity(this, layer);
            }
            else
            {

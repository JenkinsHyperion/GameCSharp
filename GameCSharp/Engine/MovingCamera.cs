...
{
    public class MovingCamera : EntityStatic, ReferenceFrame
    {


        private byte ZOOM_SPEED = 10;
        private int ZOOM_INCREMENT_PERCENT = 110;

        BoardAbstract currentBoard;
        private Graphics2D gBoard;
        ...
        static int boardHalfWidth = BoardAbstract.B_WIDTH / 2;
        static int boardHalfHeight = BoardAbstract.B_HEIGHT / 2;

        static int MAX_X = BoardAbstract.B_WIDTH;
        static int MAX_Y = BoardAbstract.B_HEIGHT;

        public static Point ORIGIN = new Point(boardHalfWidth, boardHalfHeight);
        ...
        {
            return this.zoomFactor + this.zoomDelta;
        }
	     ...
	     * @return
	     */
        public Point getAbsolutePositionOfPointOnReferenceFrame(Point2D screenPosition)
        {   //Like EntityStatic.getAbsolutePosition( Point p ), but deals with camera specific issues like zoom level and relative frame 
        ...
        public override Graphics2D getGraphics()
        {
            return this.gBoard;
        }
        ...
        public override ImageObserver getObserver()
        {
        ...
        public void drawOverlayGrid(int square, Color grid, Color axisColor)
        {
            ...
            for (int i = (pos.x - (int)(boardHalfWidth / zoomFactor) + 200) / s; i < ((pos.x + (int)(boardHalfWidth / 2 / zoomFactor) - 200) / s); i++)
            { //OPTIMIZE remove testing offset when done
            ...
            //Y AXIS
            for (int i = (pos.y - (int)(boardHalfHeight / zoomFactor) + 200) / s; i < ((pos.y + (int)(boardHalfHeight / 2 / zoomFactor)) / s); i++)
            { //OPTIMIZE remove testing offset when done
                this.gBoard.setColor(grid);

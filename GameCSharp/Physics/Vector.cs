...
namespace Game1.Physics
{
    public class Vector
    {
        ...
        public bool isShorterThan(Vector compare)
        {
        ...
        public bool isShorterThan(double length)
        {
        ...
        { //Returns the magnitude of the projection vector

            return (this.getX()) * (vector2.getX()) +
                    (this.getY()) * (vector2.getY());
        }
        public double aDot(Vector vector2)
        { //Returns the magnitude of the projection vector
        ...
        public Vector clamp()
        { // OPTIMIZE FIND POSSIBLE MATH SOLUTION FOT THIS
            if (this.getX() > 0)
            {
                return new Vector(this.x, this.y);
            }

...
{
    public class InputManagerMouseKeyboard : InputManager, KeyListener
    {

        List< KeyBinding > keysListening = new List<KeyBinding>();
	    List<KeyBinding> keysHeld = new List<KeyBinding>();
        ...
        { // being spammed from fucking keyEvent
            ...
            for (int i = 0; i < keysListening.size(); i++)
            {
                ...
                if (keysListening.get(i).keyMatch(e) && keysListening.get(i).modMatch(e))
        ...
        protected KeyCommand[] extractDirectionalKeyCommands()
        {   //Extracts and returns arrow direction events to be transferred to another input manager, namly a controller
